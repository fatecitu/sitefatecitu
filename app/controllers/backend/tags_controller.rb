class Backend::TagsController < BackendController
  ...
  def index
    ...
    @column_store = column_store_for Tag do |cm|
      cm.add :descricao
  ...
  def create
    ...
    if @tag.save
      redirect_parent_to(:action => "edit", :id => @tag)
    else
  ...
  def update
    ...
    if @tag.update_attributes(params[:tag])
      respond_to do |format|
        format.html { redirect_parent_to(:action => "edit", :id => @tag) }
        format.json { render :json => { :success => true } }
      end
    else
      respond_to do |format|
        format.html { render_to_parent(:action => "edit") }
        format.json { render :json => { :success => false, :msg => @tag.errors.full_messages.join("<br />") } }
      end
    end
  end
